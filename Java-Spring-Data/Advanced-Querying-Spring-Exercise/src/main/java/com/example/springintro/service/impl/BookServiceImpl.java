...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public List<String> findAllByEditionTypeAndCopiesLessThan() {
    ...
    public List<String> findAllByPriceLessThanOrPriceGreaterThan(BigDecimal price, BigDecimal price2) {
        return this.bookRepository.findAllByPriceLessThanOrPriceGreaterThan(price,price2)
                .stream()
    ...
    @Override
    public List<String> findAllByReleaseDateIsNot(String releaseDate) {
        return this.bookRepository.findAllByReleaseDateIsNot(releaseDate)
                .stream().map(Book::getTitle).collect(Collectors.toList());
    }
    ...
    @Override
    public List<String> findAllByTitleContainingIgnoreCase(String title) {
        return this.bookRepository.findAllByTitleContainingIgnoreCase(title)
                .stream().map(Book::getTitle).collect(Collectors.toList());
    }

    @Override
    public List<String> findAllByAuthor_LastNameStartsWith(String author_lastName) {
        return this.bookRepository.findAllByAuthor_LastNameStartsWith(author_lastName)
                .stream()
    ...
    @Override
    public Integer countAllByTitleIsGreaterThan(int number) {
        return this.bookRepository.countAllByTitleIsGreaterThan(number);
    }

    @Override
    public String findBookByTitle(String title) {
        List<Object[]> bookParts = this.bookRepository.findBookByTitle(title);
        return String.format("%s %s %s %.2f", bookParts.get(0)[0], EditionType.valueOf(String.valueOf(bookParts.get(0)[1]))
        , bookParts.get(0)[2], new BigDecimal(String.valueOf(bookParts.get(0)[3])));
    }
